...
The only ES to be covered in this section is the $(\lambda,\ \mu)$ algorithm. We have chosen it because it is one of the simplest ES and therefore easier to understand. Its design is built on top of the Abstract Generational Algorithm. We have an initial population of $\lambda$ number of individuals. The initial individuals are randomnly generated. Then, we evaluate the fitness of all individuals, as we did in the Abstract Generational Algorithm, and we calculate the best individual in the generation. Now things start to look different. For creating the new generation, the $(\lambda,\ \mu)$ algorithm performs the Truncation Selection on the parents, selecting the $\mu$ number of parents with greatest fitness. And for each parent, a $\lambda \div \mu$ number of children are generated. The children are created by mutating a copy of their parent. This whole process is done until we arrive at an optimal solution or the maximum time runs out. The pseudocode of the $(\lambda,\ \mu)$ algorithm is shown as follows.

\begin{algorithm}[H]
    \caption{The $(\lambda,\ \mu)$ Evolution Strategy}
    \begin{algorithmic}[1]
        \Procedure {LambdaMuES}{$\lambda$, $\mu$}
            \State {$best\gets \epsilon$}
            \State {$P \gets \{ \}$} 
