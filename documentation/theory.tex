...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        \Procedure {GreedyAlgorithm}{candidates}
            ...
            \While {$\ !isEmpty(candidates)\ \&\&\ !found$}
...
Next, we will solve the Bootaku problem using a greedy algorithm. The greedy algorithm that we are going to use is inspired by the one defined in \cite{book-greedy} for solving the \textit{Assignment of tasks} problem.

\begin{algorithm}[H]
    \caption{Greedy Algorithm for the Bootaku problem}
    \begin{algorithmic}[1]
        \Procedure {GreedyBootaku}{profits, assignments}
            \State {$freelancer \gets \epsilon$} 
            \State {$review\gets \epsilon$}
            \For {$\ freelancer\ 1\ to\ n$}
                \For {$\ review\ 1\ to\ m$}
                    \State {$assignments[freelancer,\ review] = false$} \Comment{We initialise the assignments matrix (to false or cero, it does not matter)}
                \EndFor
            \EndFor
            \For {$\ review\ 1\ to\ m$}
                \State {$freelancer \gets bestFreelancerFor(profits,\ assignments,\ review)$}
                \State {$assignments[freelancer,\ review] = true$} \Comment{Again, it can be true or one, depending on the implementation}
            \EndFor
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        \Procedure {bestFreelancerFor}{profits, assignments, review}
            ...
            \For {$\ freelancer\ 1\ to\ n$}
                \If {$\ profits[freelancer,\ review] < min$}
                    \State {$min \gets profits[freelancer,\ review]$}
                    \State {$best \gets freelancer$}
                \EndIf
            \EndFor
